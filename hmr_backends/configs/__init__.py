...
def get_config(config_file: str, merge: bool = True, update_cachedir: bool = False) -> CN:
    ...
    Used by both HAMER/WiLoR/HTM and HAWOR model loading.
    The update_cachedir logic handles the '_DATA' cache directory pattern
    shared by all backends.

    Args:
      config_file: Path to the YAML config file.
      merge: Whether to merge with the default config or not.
      update_cachedir: Whether to resolve relative MANO paths under CACHE_DIR.
    Returns:
    ...
    if update_cachedir:
      def update_path(path: str) -> str:
        # If the path already contains the cache dir basename, leave it as-is
        if os.path.basename(CACHE_DIR) in path:
          return path
...
CACHE_DIR_HAMER = CACHE_DIR
CACHE_DIR_HAWOR = CACHE_DIR
