...
class HaworBackendRunner(BaseBackendRunner):
    def infer(self, inputs):
        ...
        for hand_side in ('left', 'right'):
            segments = inputs.segments_by_hand[hand_side]
            for segment in tqdm(segments, desc=f"Pass 3: Running HAWOR-{hand_side.upper()}"):
                seg_instances = [inputs.instances_by_key[(idx, hand_side)] for idx in segment]
                seg_imgfiles = [str(inputs.img_paths[idx]) for idx in segment]
                ...
                for local_i, inst in enumerate(seg_instances):
                    ...
                    outputs.append(BackendOutputInstance(
                        ...
                        raw_backend_meta={
                            ...
                            'segment_range': [int(segment[0]), int(segment[-1])],
                        },
                    ))
        return outputs
