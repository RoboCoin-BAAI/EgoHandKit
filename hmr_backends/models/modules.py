"""HaWoR-specific model sub-modules.

NOTE: This module contains MANOTransformerDecoderHead with HARDCODED transformer
parameters (depth=6, heads=8, etc.) and a simplified forward() that returns
(pred_pose, pred_shape, pred_cam). This differs from heads/mano_head.py which
supports configurable IEF_ITERS, JOINT_REP_TYPE, and TRANSFORMER_INPUT.

Also contains temporal_attention and PositionalEncoding for HaWoR temporal modeling.
Derived from HaWoR's lib/models/modules.py with imports rewritten to use the hmr_backends.* namespace.
"""
