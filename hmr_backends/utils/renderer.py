...
class Renderer:
    ...
    def render_rgba(
        ...
        # self.add_point_lighting(scene, camera_node)  # PointLight 亮度 ∝ 1/d², 不同 backend 的 mesh 深度差异导致亮度不一致
    ...
    def render_rgba_multiple(
        ...
        # Support per-mesh colors: mesh_base_color can be a single tuple or a list of tuples
        if isinstance(mesh_base_color, list) and len(mesh_base_color) == len(vertices):
            colors = mesh_base_color
        else:
            colors = [mesh_base_color] * len(vertices)

        mesh_list = [pyrender.Mesh.from_trimesh(self.vertices_to_trimesh(vvv, ttt.copy(), ccc, rot_axis, rot_angle, is_right=sss)) for vvv,ttt,sss,ccc in zip(vertices, cam_t, is_right, colors)]
