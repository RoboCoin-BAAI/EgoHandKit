...
# ---------------------------------------------------------------------------
# HaWoR-style bbox / crop helpers (originally from hmr_backends/utils/imutils.py)
# These are used by TrackDatasetEval for HaWoR inference.
# ---------------------------------------------------------------------------

def imutils_crop(img, center, scale, res, rot=0):
    """Crop image according to the supplied bounding box (skimage-based).

    This is the HaWoR-style crop, which uses skimage.transform.resize.
    The existing `crop_img` above uses cv2-based patch extraction instead.
    """
    ul = np.array(transform([1, 1], center, scale, res, invert=1, as_int=True)) - 1
    br = np.array(transform([res[0] + 1, res[1] + 1], center, scale, res, invert=1, as_int=True)) - 1
...
def boxes_2_cs(boxes):
    """Convert (N,4) [x1,y1,x2,y2] boxes to centers and scales."""
...
def box_2_cs(box):
    """Convert a single [x1,y1,x2,y2] box to center and scale."""
...
def est_intrinsics(img_shape):
    ...
    h, w, c = img_shape
    img_center = np.array([w / 2.0, h / 2.0]).astype(np.float32)
    img_focal = np.sqrt(h ** 2 + w ** 2).astype(np.float32)
