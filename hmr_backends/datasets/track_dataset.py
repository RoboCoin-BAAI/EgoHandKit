"""
Track Dataset for HaWoR inference.
Derived from HaWoR's lib/datasets/track_dataset.py with imports
rewritten to use the hmr_backends.* package namespace.
"""
...
from hmr_backends.core.constants import IMG_NORM_MEAN, IMG_NORM_STD
from hmr_backends.datasets.utils import imutils_crop as crop, boxes_2_cs
...
class TrackDatasetEval(Dataset):
    ...
    def __init__(self, imgfiles, boxes,
        ...
        self.normalize_img = Compose([
            ToTensor(),
            Normalize(mean=IMG_NORM_MEAN, std=IMG_NORM_STD)
        ])
