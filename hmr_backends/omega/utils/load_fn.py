...
from hmr_backends.omega.preprocess import compute_preprocess_meta
...
def load_and_preprocess_images_with_meta(image_path_list, mode="balanced", image_resolution=512, patch_size=16):
    ...
    padded_hw = tuple(int(x) for x in images.shape[-2:])
    metas = []
    for image_path in image_path_list:
